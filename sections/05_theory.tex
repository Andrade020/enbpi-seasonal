...
\begin{enumerate}
  \item Theorem~\ref{thm:xu_original} uses $T$ (total training
    ...
    converges at the slower rate $O(\sqrt{\Sper}\cdot T^{-1/2})$
    due to the smaller effective sample size $\Ts = T/\Sper$.
...
\begin{enumerate}
  \item[\textup{(i)}]
    ...
    \[
      ...
      \;=\; 2\bsstar.
    \]
    Hence the coverage guarantee for pooled \texttt{EnbPI} is
    bounded below by $2\bsstar > 0$ for all $T$: no amount of
  ...
  \item[\textup{(iii)}]
    Consequently, whenever $\bsstar > 0$, the coverage guarantee
    for pooled \texttt{EnbPI} remains bounded below by
    $2\bsstar > 0$ for all $T$, while the stratified guarantee
    converges to zero.  By Lemma~\ref{lem:bias_properties}(i),
    $\bsstar > 0$ if and only if not all seasonal scales are equal.
    Stratification therefore provides a strictly tighter theoretical
    guarantee than pooling whenever seasonal heteroskedasticity
    is present.
\end{enumerate}
\end{proposition}
...
\begin{proof}
  ...
  $12\sqrt{\log(16T)/T} \to 0$ and the estimation term
  ...
  Lemma~\ref{lem:bias_properties}(i).
\end{proof}
...
is more informative.  However, for the food-IPCA application with
$T = 240$ months of training data ($\Ts = 20$), the seasonal bias
