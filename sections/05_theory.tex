...
\begin{enumerate}
  ...
  \item Theorem~\ref{thm:xu_original} is \emph{only valid} when
    ...
    which fails under seasonal heteroskedasticity.
    Proposition~\ref{prop:pooled_gap} adds the seasonal bias
    $2\bsstar$ (and adjusts the sampling constant) on the
    right-hand side of Theorem~\ref{thm:xu_original}, yielding a
    bound that is valid but does
...
there is a genuine trade-off: stratification removes the
...
additional sampling variance.  Since the pooled sampling and
estimation terms are nonnegative (and $L \geq L_{\sstar}$), a
conservative sufficient condition for the stratified
bound~\eqref{eq:strat_bound} to be smaller than the pooled
bound~\eqref{eq:pooled_gap} is:
\begin{equation}
...
$\deltaTs \geq \deltaT$ (the typical case, since the stratum
...
$\bsstar$); the condition therefore cannot be satisfied at
$T = 120$.  This is unsurprising: worst-case bounds of this
type are far from tight at such small stratum sizes, so the
bounds alone cannot adjudicate between methods at $T = 120$;
the asymptotic comparison in
Proposition~\ref{prop:strat_dominance} is more informative.
For the food-IPCA application with $T = 240$ months of training
data ($\Ts = 20$), a seasonal bias of the magnitude illustrated
in Section~\ref{sec:model:seasonal} ($b_H \approx 0.12$ in the
Gaussian example) is a fixed quantity that does not shrink with
$T$, while the sampling terms do.  The simulation in
Section~\ref{sec:simulations} shows that \texttt{EnbPI-S}
achieves better high-volatility conditional coverage than
pooled EnbPI from $T = 240$ months onwards
(Table~\ref{tab:e1_coverage_T240}), validating the asymptotic
theory at moderate sample sizes.
\end{remark}
...
where:
\begin{itemize}
  \item $\Delta_{\sstar}(T) = \lambda\bigl(\hat{C}_{T+1}^{\alpha,\textup{strat}}
