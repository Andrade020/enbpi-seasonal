...
This section characterises the coverage error incurred when EnbPI is
applied with a pooled calibration buffer under
Assumption~\ref{ass:seasonal_general}.  The main result,
...
Before the result is stated, it is worth clarifying how the pooled
empirical CDF relates to the seasonal CDF of the prediction target.
...
\]
Under Assumption~\ref{ass:seasonal_general} and good estimation quality, this
...
\label{lem:coupling}
Under Assumption~\ref{ass:seasonal_general} and the model~\eqref{eq:dgp},
when $X_{T+1} = x_{T+1}$:
\begin{equation}
  \label{eq:coupling}
  \hat{\epsilon}_{T+1}
...
\citet{xu2023conformal}.  The difference in the present setting is
that $\epsilon_{T+1} \sim F_{\sstar}$, which is season-specific,
while the pooled buffer $\hat{\boldsymbol\epsilon}$ contains
residuals drawn from all $\Sper$ seasonal distributions.
...
Let Assumptions~\ref{ass:xu_a2} and~\ref{ass:seasonal_general} hold,
so that the errors are independent, identically distributed within
each season according to a continuous $F_s$, and $F_{\sstar}$ and each
$F_s$ are Lipschitz with constant $L = \max_s L_s$.  The seasonal
scale mixture of Assumption~\ref{ass:seasonal} is \emph{not} required;
under it, $L = L_G/\min_s \sigma_s$.
...
whenever $\deltaT \to 0$.  Term \textup{(iii)} is constant in $T$, and
is positive whenever the season-$\sstar$ error distribution differs
from the pooled mixture at any point.  Under the additional structure
of Assumption~\ref{ass:seasonal},
Lemma~\ref{lem:bias_properties}(iii) makes that concrete:
...
applied under the seasonal model.  The full argument is deferred to
Appendix~\ref{app:proofs}; what follows is a sketch that highlights
the source of the additional term~(iii).
...
by the Probability Integral Transform, since $\epsilon_{T+1} \sim F_{\sstar}$
and $F_{\sstar}$ is continuous.
...
Term (A) is bounded via Lemma~2 of \citet{xu2023conformal},
using the LOO coupling~\eqref{eq:coupling} and
Assumption~\ref{ass:xu_a2}, which yields a contribution of order
$(L+1)\deltaT^{2/3}$.

Term (B) requires more care.  The pooled empirical CDF
$\tilde{F}_T^{\text{pool}}$ is formed from $T$ \emph{independent} but
not identically distributed observations, $T_s = T/\Sper$ from each
$F_s$, with expectation $\bar{F}$.  The sharp
Dvoretzky--Kiefer--Wolfowitz (DKW) inequality of
\citet{massart1990tight} applies only to i.i.d.\ samples, so that it
cannot be invoked directly here.  Lemma~\ref{lem:dkw_noniid} in
Appendix~\ref{app:proofs} establishes instead, by symmetrisation and
...
When the seasons do not differ, that is when $F_s = F$ for every $s$,
which under Assumption~\ref{ass:seasonal} means
$\sigma_s = \sigma$ throughout, we have $\bar{F} = F_{\sstar}$ and
$\bsstar = 0$;
...
can be read as quantifying the loss from \emph{not} applying
Mondrian-type stratification in a time-series setting where
exchangeability fails.  Theorem~\ref{thm:strat_coverage}
...
Proposition~\ref{prop:pooled_gap} identifies a limitation of pooled
calibration in seasonally heteroskedastic series that is \emph{not}
resolved by increasing the sample size, and three practical
implications follow from it.

The first is persistent undercoverage in high-volatility seasons.
When $\sigma_{\sstar}$ exceeds the remaining seasonal scales, the
pooled quantiles underestimate the true dispersion of errors in that
season, which produces intervals that are systematically too narrow;
the converse holds for low-volatility seasons, which receive wider
intervals than necessary.

The second is asymptotic invalidity.  Even as $T \to \infty$ and
terms~(i) and~(ii) in~\eqref{eq:pooled_gap} vanish, the \emph{upper
bound} remains bounded below by $2\bsstar > 0$.  An upper bound alone
does not prove that the realised gap stays away from zero; the
limiting-coverage computation in Step~6 of
Appendix~\ref{app:proofs:prop_pooled}
(equation~\eqref{eq:cov_approx}) does show, however, that pooled
coverage converges to
...
Section~\ref{sec:simulations} confirm a persistent per-season gap.
In this sense pooled EnbPI is \emph{asymptotically invalid} in the
presence of genuine seasonal heteroskedasticity.

The third implication is more constructive: the seasonal bias
$\bsstar$ can be estimated consistently from data, using the
season-specific and pooled empirical CDFs of the LOO residuals,
$\hat{b}_{\sstar} = \sup_x |\hat{F}_{T,\sstar}(x) - \hat{F}_T^{\text{pool}}(x)|$,
where $\hat{F}_{T,\sstar}$ is the empirical CDF restricted to season
$\sstar$.  This estimator is consistent by the DKW inequality applied
separately to the two empirical processes, so that a practitioner can
detect seasonal bias before deploying prediction intervals.
