...
% ============================================================
\section{Coverage Gap of Pooled \texttt{EnbPI} Under Seasonal
         Heteroskedasticity}
...
\begin{align}
  ...
  \bigr| \notag\\
  &\quad\leq\;
  \underbrace{12\sqrt{\frac{\log(16T)}{T}}}_{\text{(i) sampling error}}
  \;+\;
...
and satisfies $\bsstar > 0$ whenever $\sigma_{\sstar} \neq \sigma_s$
for some $s \neq \sstar$.
\end{proposition}
...
$\bar{F}$.  Using the decomposition
$\tilde{F}_T^{\text{pool}}(x) - \bar{F}(x)
= \frac{1}{\Sper}\sum_{s=1}^\Sper
  [\tilde{F}_{T_s,s}(x) - F_s(x)]$,
where $\tilde{F}_{T_s,s}$ is the season-$s$ empirical CDF formed
from the $T_s$ i.i.d.\ observations in stratum $s$, we obtain:
\[
  \sup_x\bigl|\tilde{F}_T^{\text{pool}}(x) - \bar{F}(x)\bigr|
  \;\leq\;
  \max_{s}\sup_x\bigl|\tilde{F}_{T_s,s}(x) - F_s(x)\bigr|.
\]
Each term on the right satisfies the DKW inequality
\citep{massart1990tight} with $T_s$ i.i.d.\ samples.  Applying a
union bound over $\Sper$ seasons yields a contribution of order
$\sqrt{\log(16T_s)/T_s} = \sqrt{\Sper\log(16T/\Sper)/T}$,
which converges to zero as $T\to\infty$ for fixed $\Sper$.
The full proof in Appendix~\ref{app:proofs} carries out this
argument precisely.  The key step is to apply the DKW inequality
directly to the $T$ mutually independent (though not identically
distributed) errors pooled across all seasons; the DKW bound
$\Prob(\sup_x|\tilde{F}_T^{\text{pool}}(x)-\bar{F}(x)|>u)
\leq 2e^{-2Tu^2}$ holds for any sequence of independent random
variables \citep{massart1990tight}, not just i.i.d.\ ones,
yielding the $T$-based term~(i) with constant $12$ as in
\citet{xu2023conformal}.
...
yields~\eqref{eq:pooled_gap}.  The constants in
terms~(i) and~(ii) carry over exactly from
\citeauthor{xu2023conformal}'s proof.
\end{proof}
...
\emph{Mondrian conformal prediction} \citep{vovk2005mondrian}.
...
\item \textbf{Persistent undercoverage in high-volatility seasons.}
  When $\sigma_{\sstar} > \bar{\sigma}$ (a high-volatility season),
...
\item \textbf{Asymptotic invalidity.}
  ...
  in~\eqref{eq:pooled_gap} vanish, the gap remains bounded below by
  $\bsstar > 0$.  Pooled EnbPI is \emph{asymptotically invalid}
  in the presence of genuine seasonal heteroskedasticity.
