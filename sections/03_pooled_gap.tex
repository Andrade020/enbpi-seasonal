...
argument precisely and verifies that the multiplicative constant
in term~(i) of~\eqref{eq:pooled_gap} is $12$, as in
\citet{xu2023conformal}.\footnote{%
  For fixed $\Sper$, the factor $\sqrt{\Sper\log(16T/\Sper)/T}$
  has the same asymptotic order as $\sqrt{\log(16T)/T}$, so the
  rate of convergence of term~(i) is $O(T^{-1/2})$ in both cases.
  The precise constant differs by a factor of $\sqrt{\Sper}$;
  this is the price of pooling non-homogeneous data.
}
