...
\item \emph{Theoretical guarantees.}
  ...
  We also derive an asymptotic dominance result
  (Proposition~\ref{prop:strat_dominance}) and a
  finite-sample dominance condition
  \eqref{eq:dominance_condition} under which \texttt{EnbPI-S}
  has a strictly tighter bound than pooled EnbPI, and a bound
  showing that the \texttt{EnbPI-S} interval converges to the
  season-specific oracle interval
...
$T_s = 35$), both methods achieve coverage within $3.3$
percentage points of nominal (on opposite sides), and
the width ratio rises to $2.3\times$:
\texttt{EnbPI-S} assigns a $13.8\%$ interval to April
(the peak of the soybean harvest-export season, highest
forecast risk) and a $6.0\%$ interval to September
(the most predictable month), while
...
leave-one-out (LOO) bootstrap ensemble and a sliding window update.
\citet{xu2023sequential} extended it to exploit residual
autocorrelation (Sequential Predictive Conformal Inference,
SPCI), improving efficiency under serial dependence.  \citet{barber2023conformal} established coverage
...
The SARIMA mean structure closely approximates that of our
data-generating process (DGP; see
Section~\ref{sec:simulations}), so its failure (September
coverage of $66.2\%$ against the $90\%$ nominal) is
attributable almost entirely to two compounding
...
quantiles ($+8.9$ pp on high-volatility coverage);
\texttt{EnbPI-S} corrects both ($+5.5$ pp further),
reducing the cross-season coverage spread more than
twelve-fold relative to SARIMA ($0.019$ vs $0.243$).
