...
with marginal distributions $F_{s(t)}$ (Assumption~\ref{ass:seasonal}).
The DKW inequality \citep{massart1990tight} holds for any sequence
of independent random variables, whether or not they are identically
distributed: for $T$ independent observations the pooled empirical CDF
$\tilde{F}_T^{\text{pool}}(x) = T^{-1}\sum_{t=1}^T\Ind{\epsilon_t\leq x}$
satisfies
\[
  \Prob\!\Bigl(\sup_x\bigl|\tilde{F}_T^{\text{pool}}(x)-\bar{F}(x)\bigr| > u\Bigr)
  \;\leq\; 2e^{-2Tu^2},
\]
since $E[\tilde{F}_T^{\text{pool}}(x)] = T^{-1}\sum_{t=1}^T F_{s(t)}(x) = \bar{F}(x)$
(balanced panel) and the bounded-difference condition with
$c_t = 2/T$ applies regardless of the marginal distributions.
Define $v_T = \sqrt{W(16T)}/(2\sqrt{T}) \leq \sqrt{\log(16T)/T}$
and the good event $A_T = \{\sup_x|\tilde{F}_T^{\text{pool}}(x)-\bar{F}(x)| \leq v_T\}$,
so that $\Prob(A_T^c) \leq \sqrt{\log(16T)/T}$.
On the event $A_T$:
\[
  \sup_x\bigl|\tilde{F}_T^{\text{pool}}(x) - \bar{F}(x)\bigr|
  \;\leq\; v_T \;\leq\; \sqrt{\frac{\log(16T)}{T}}.
\]
