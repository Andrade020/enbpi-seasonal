...
This appendix collects the proofs of all stated results.  For
clarity, the notation of \citet{xu2023conformal} is followed
throughout, and the points at which the seasonal structure
introduces differences are identified explicitly.  Without loss of
generality, the results are proved for $t = T+1$ (one-step-ahead
prediction); extensions to $t > T+1$ hold by the same argument
whenever Assumptions~\ref{ass:strat_a1}--\ref{ass:strat_a2}
...
i.i.d.\ samples, whereas the pooled empirical CDF under
Assumption~\ref{ass:seasonal_general} aggregates independent but
\emph{not} identically distributed errors.  The following standard
substitute is therefore recorded, proved by symmetrisation and the
...
The argument follows the proof of
\citet[Theorem~1]{xu2023conformal}, modified to account for the
heterogeneous seasonal distributions.
...
argument is replaced by the mixture $\bar{F}$, Lipschitz with
constant $L = \max_s L_s$, while Assumption~\ref{ass:xu_a2} is used
unchanged:
\[
...
(Assumption~\ref{ass:seasonal_general}), and under the balanced panel
$E[\tilde{F}_T^{\text{pool}}(x)]
= T^{-1}\sum_{t=1}^T F_{s(t)}(x) = \bar{F}(x)$.
Since the errors are not identically distributed, the sharp DKW
inequality of \citet{massart1990tight} is not available, and
Lemma~\ref{lem:dkw_noniid} is invoked instead.  Define the good event
\[
  A_T \;=\;
...
The symmetric difference $\mathcal{D}_{\sstar}(T)$ between
